...
<#if clazz.identifierProperty?has_content>    
    public ${declarationName} findById( ${pojo.getJavaTypeName(clazz.identifierProperty, true)} id) {
...
<#if clazz.identifierProperty?has_content>
    public ${declarationName} findById( ${c2j.getJavaTypeName(clazz.identifierProperty, true)} id) {
        log.debug("getting ${declarationName} instance with id: " + id);
        try {
...
<#if true && methname.startsWith("find")>
