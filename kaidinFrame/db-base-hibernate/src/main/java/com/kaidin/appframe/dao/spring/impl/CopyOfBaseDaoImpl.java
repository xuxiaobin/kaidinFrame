 ...
 */
@Transactional
public abstract class CopyOfBaseDaoImpl<T extends BaseEntity> extends HibernateDaoSupport implements IBaseDao<T> {
	private static final transient Logger logger = LoggerFactory.getLogger(CopyOfBaseDaoImpl.class);
	private static final int MAX_QUERY_LIMIT = 10000;	// 最大的查询数据条数限制，默认10000
	...
	public CopyOfBaseDaoImpl(Class<T> clazz) throws AppframeException {
		entityClass = clazz;
...
//				save(entity);
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@SuppressWarnings("unchecked")
	public List<T> queryEntities(String hqlWhere, final int rowIndex, int rowNum) throws AppframeException {
		List<T> result = null;
		...
		try {
			...
			final String queryString = "from " + entityClassName + " where " + hqlWhere;
			hibernateTemplate.setMaxResults(MAX_QUERY_LIMIT < rowNum ? MAX_QUERY_LIMIT: rowNum);
			result = hibernateTemplate.execute(
					new HibernateCallback<List<T>>() {
						public List<T> doInHibernate(Session session) throws HibernateException {
							Query query = session.createQuery(queryString);
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@SuppressWarnings("unchecked")
	public List<T> queryEntities(final String hqlWhere, final String[] names, final Object[] values, final int rowIndex, int rowNum) throws AppframeException {
		List<T> result = null;
	...
	public DataContainer<T> queryEntities(final PageLoadConfig pageLoadCfg) throws AppframeException {
		...
		try {
			...
			if (0 < totalCount) {
				...
				hibernateTemplate.setMaxResults(MAX_QUERY_LIMIT < pageLoadCfg.getLimit() ? MAX_QUERY_LIMIT: pageLoadCfg.getLimit());
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@SuppressWarnings("rawtypes")
	public List queryByFullHql(final String hql, final int rowIndex, int rowNum) throws AppframeException {
		List result = null;
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@SuppressWarnings("rawtypes")
	public List queryByFullHql(final String hql, final String[] names, final Object[] values, final int rowIndex, int rowNum) throws AppframeException {
		List result = null;
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@SuppressWarnings("rawtypes")
	public List queryNativeSql(final String sql, final String[] names, final Object[] values, final int rowIndex, int rowNum) throws AppframeException {
		List result = null;
	...
	public List queryByFullHqlNoLimit(final String hql, final String[] names, final Object[] values, final int rowIndex, int rowNum) throws AppframeException {
		List<T> result = null;
