...
//					result.setErrCode(GuiConstType.ErrCode.USER_LOCKED);
//					result.setErrMsg("用户错误登陆次数过多，已被锁定");
...
//							// 这里root不能被锁，否则没法解锁了
//							result.setErrMsg("密码不正确，已经尝试第" + status + "次密码");
//						} else {
//							if (GuiConstType.Status.ATTEMPT_MAX_TIMES <= status) {
//								// 尝试的次数达到最大限制后用户被锁定
//								status = GuiConstType.Status.LOCK;
//								result.setErrMsg("密码不正确，已经尝试超过" + GuiConstType.Status.ATTEMPT_MAX_TIMES + "，账号已被锁定");
//							} else {
//								int count = GuiConstType.Status.ATTEMPT_MAX_TIMES - status;
//								result.setErrMsg("密码不正确，再尝试" + count + "次账号将被锁定");
//							}
//						}
//					}
//					user.setStatus(status); // 记录错误密码尝试次数
//					result.setErrCode(GuiConstType.ErrorCode.PASSWD_ERR);
//				}
//				userDao.update(user);
//			} else {
//				result.setErrCode(GuiConstType.ErrorCode.USER_NOT_EXIST);
